...
## 十五、外链隔离与 Agent 深度搜索（2026-08-26）

- 修复对话/Agent 搜索结果中的外部链接接管 Electron 主窗口的问题：`setWindowOpenHandler` 拒绝应用内弹窗，`will-navigate` 阻止主窗口跳离本地 SPA；公网 HTTP(S) 链接统一交给系统默认浏览器打开。
- Agent 联网工具升级为并行多源深度搜索：保留 AnySearch / SearXNG 配置源，同时并行调用 Bing RSS / Bing HTML / DuckDuckGo 和 arXiv / Crossref / OpenAlex 开放学术源。
- 新增 URL 规范化去重、中文二元组相关性匹配、来源共识、学术域名权重和新鲜度轻量重排；单一搜索源失败或超时不阻断整体结果。
- 新增公开网页正文取证：自动提取少量高排名页面正文作为证据；解析前校验 HTTP(S)、DNS 解析结果与重定向目标均为公网地址，避免访问本机/内网服务。
- Agent 提示链路不再把完整搜索 JSON 塞回模型，改为最多约 8000 字符的编号证据摘录；要求模型使用 `[编号]` 引用，并在回答末尾输出完整来源链接。
- 清理“搜索陶瓷材料相关的学术名词”等口语意图词，让实际检索关键词聚焦到“陶瓷材料”。
- 实测“陶瓷材料”深度搜索约 11 秒返回 8 条去重排序结果；后端 49/49 测试通过，前端生产构建通过，v0.3.4 安装包已重建。
